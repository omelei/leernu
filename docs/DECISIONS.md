...
## ADR-106 — The five boxes keep their intervals, a review lands on a calendar day in Amsterdam, and a mistake under a clock is marked rather than punished

Status: accepted, 2026-09-12. Part of the house style v2 handoff
(`docs/huisstijl-v2/00-inventarisatie.md` §4).

### Context

The handoff asks for five Leitner boxes, a next review per item, and a
"controleren" mark, and leaves the intervals and the promotion rule open. The
code has had five boxes at 1, 2, 4, 8 and 21 days since ADR-005, with the next
review set to `now + interval × 24h`. That put an item practised at seven in
the evening out of reach until seven the next evening, and moved it by an
hour across every change to or from summer time.

### Decision

- The intervals stay **1, 2, 4, 8 and 21 days**. Changing them would give every
  existing child's boxes a different meaning.
- Right, without a clock: one box up, to five at most; a "controleren" mark is
  cleared. Wrong, without a clock: back to box one; the mark is cleared.
- Right under a clock: one box up. Wrong under a clock: the box stays and the
  item is marked "controleren", which puts it first in the next round without
  a clock whatever its date. No round with a clock is built yet (stap 8 is out
  of scope); the rule is in `game-core` so that building one later is a flag
  and not a change to the scheduler.
- The next review is **the start of the calendar day in Europe/Amsterdam**
  that lies _interval_ days after the day of the answer.

### Consequences

No data migration: stored `volgendeReview` values stay valid instants, and the
new rule applies from the next answer. Items come round a few hours earlier
than before. `game-core` gains a small calendar module that works in
Europe/Amsterdam through `Intl`, which is available to a server as well as to
a browser, so ADR-015 holds.

---

## ADR-107 — The dot shows the share of a set a child remembers, and "remembers" means right last time and not yet due

Status: accepted, 2026-09-12. Part of the house style v2 handoff
(`docs/huisstijl-v2/00-inventarisatie.md` §5).

### Context

The handoff makes the dot the product's mark and says it shows retention and
nothing else, and it leaves the arithmetic open. `retention.ts` forecast
`0.9^(days / interval)` per item and averaged it. That figure stood next to a
count ("9 / 12 onthouden") that was computed differently, so one card could say
two things about one idea; it had never been measured against anything; and an
item answered wrong a minute ago scored 100%, because it had just been
reviewed.

### Decision

An item is **remembered** when its last answer was right (box two or higher)
and its next review has not yet arrived. A set's **retention** is the share of
its items that are remembered, as a whole percentage. At a later moment — the
horizon of one and three weeks on the test card — it is the same count with
that moment instead of now. A set with no items, or whose items have never been
answered, **has no retention**, and no dot is drawn for it.

### Consequences

The dot and the sentence beside it are the same number: nine of twelve is a
dot three quarters full. The value moves one item at a time rather than
smoothly. The three-week horizon is low for a beginner, because only items in
box five are still not due by then, which is what the dot is meant to show.
`countMastered` (box five) stops meaning "remembered"; the stamp that asks for
every item in box five keeps asking for that.
